...
When deining a base model and a reward function, it is recommended to register them so that they can be easily combined using ``diffusiongym.make``.
You can register them as follows:

.. code-block:: python
   :emphasize-lines: 1,3,7

   from diffusiongym import base_model_registry, reward_registry
