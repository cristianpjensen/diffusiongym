...
class OptimalTransportScheduler(Scheduler):
...
class DiffusionScheduler(Scheduler):
    ...
    def alpha(self, t: torch.Tensor) -> torch.Tensor:
        r""":math:`\alpha_t`."""
        k = self._get_index(t)
        return torch.sqrt(self.alpha_bar[k])

    def beta(self, t: torch.Tensor) -> torch.Tensor:
        r""":math:`\beta_t`."""
        k = self._get_index(t)
        return torch.sqrt(1 - self.alpha_bar[k])

    def alpha_dot(self, t: torch.Tensor) -> torch.Tensor:
        r""":math:`\dot{\alpha}_t`."""
        k = self._get_index(t)
        return 0.5 * self.alpha_bar_dot[k] / self.alpha(t)

    def beta_dot(self, t: torch.Tensor) -> torch.Tensor:
        r""":math:`\dot{\beta}_t`."""
        k = self._get_index(t)
        return -0.5 * self.alpha_bar_dot[k] / self.beta(t)
